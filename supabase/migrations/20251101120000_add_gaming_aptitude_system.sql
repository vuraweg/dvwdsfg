/*
  ...
  1. New Tables
    - `gaming_companies` - Stores company information (Accenture, Cognizant, Capgemini)
      ...
      - `secondary_color` (text, hex color for branding)
      - `difficulty_modifier` (numeric, affects scoring)
      - `is_active` (boolean, enable/disable company)
      - `display_order` (integer, ordering on landing page)
      - `created_at` (timestamptz)

    - `game_levels` - Defines level configurations for each company
      - `id` (uuid, primary key)
      - `company_id` (uuid, foreign key to gaming_companies)
      - `level_number` (integer, 1-4)
      - `grid_size` (integer, 4, 5, 6, or 7)
      - `time_limit_seconds` (integer, time allowed)
      - `target_score` (integer, minimum score to unlock next level)
      - `obstacle_density` (numeric, percentage of grid with obstacles)
      - `is_active` (boolean)
      - `created_at` (timestamptz)
    ...
    - `user_game_progress` - Tracks user progress across companies and levels
      ...
      - `company_id` (uuid, foreign key to gaming_companies)
      - `level_id` (uuid, foreign key to game_levels)
      - `is_unlocked` (boolean, level unlock status)
      - `is_completed` (boolean, level completion status)
      - `best_score` (integer, user's best score for this level)
    ...
    - `game_scores` - Stores individual game attempt scores
      ...
      - `completion_time_seconds` (integer, time taken)
      - `path_length` (integer, number of moves)
      - `optimal_path_length` (integer, theoretical optimal path)
      - `efficiency_percentage` (numeric, path efficiency)
      - `is_valid` (boolean, server-side validation)
      - `completed_at` (timestamptz, when game completed)
      - `created_at` (timestamptz)

    - `leaderboards` - Maintains rankings
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to user_profiles)
      - `company_id` (uuid, foreign key to gaming_companies, nullable for global)
      - `level_id` (uuid, foreign key to game_levels, nullable for overall)
      - `total_score` (integer, cumulative score)
      - `rank` (integer, user's rank)
      - `percentile` (numeric, performance percentile)
    ...
    - `game_sessions` - Track active sessions and prevent cheating
      ...
      - `user_id` (uuid, foreign key to user_profiles)
      - `level_id` (uuid, foreign key to game_levels)
      - `session_token` (uuid, unique session token)
      - `grid_data` (jsonb, serialized grid state)
      - `start_time` (timestamptz, session start)
      - `expected_end_time` (timestamptz, based on time limit)
      - `actual_end_time` (timestamptz, when completed)
      - `is_active` (boolean, session status)
      - `is_validated` (boolean, server validation status)
